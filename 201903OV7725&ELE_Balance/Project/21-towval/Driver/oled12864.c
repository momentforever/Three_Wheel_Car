...
{
  int i;
  uint8 x,y,temp,imgdrawbuff[4800];
  for(i=0;i<4800;i++)
  {
    imgdrawbuff[i]=img[i];         
  }
  ...
  for(y=0;y<8;y++)
  {
    ...
    OLED_WrCmd(0x10);      //0x10+0~16表示将128列分成16组其地址所在第几组
     ...
     for(x=0;x<80;x++)
     {
       ...
       for(i=0;i<8;i++)
       {
         temp+=imgdrawbuff[y*640+i*80+x]<<(7-i); //行转换为列
         if(y==7&&i==3) break;
       }
...
{       
    ...
    Digits[0] = data/BaseSize;
    for( int i=1;i<DataSize-1;i++ )
    {
      Digits[i] = ( data%BaseSize ) /(BaseSize/10);
      BaseSize/=10;
    }
    Digits[DataSize-1]=data%10;
    for( int i=0;i<DataSize;i++ )
    {
      OLED_P6x8Char(Digits[i]+48);
    }
