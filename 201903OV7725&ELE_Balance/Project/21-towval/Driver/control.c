...
void Direction_ADControl_zl()
{
  ...
  if((rightM==1) || (leftM==1)){
    ...
    if((rightM==1) && (Mh>30)) {PID_AD_TURN.OUT=-0.15;} //100ms后转向
    if((leftM==1)  && (Mh>100)){PID_AD_TURN.OUT= 0.17;}
    
    
    if(Mh>300) {  //250ms后转向完成
  ...
  if((rightM==4) || (leftM==4)){
    ...
    if((rightM==4) && (Mh<100)) {PID_AD_TURN.OUT=-0.15;}    //右拐100ms
    else if((leftM==4)  && (Mh<100)){PID_AD_TURN.OUT= 0.17;}
    else if((100<=Mh) && (Mh<1200)){   //按照正常直道走 200ms
      
    }
    else if(1200<=Mh){ //300ms后恢复欧拇环判断初始标志
       Mh=0; 
 ...
 */
  ...
  if (runmode == 1) //runmode=1，三轮跑,不需要直立分量
  {
    if (CarmeraMiss == 1 && !AD_Miss) //电磁工作
    {
      ...
      LeftMotorOut = -PID_SPEED.OUT - PID_AD_TURN.OUT;  //电磁分量
      RightMotorOut = -PID_SPEED.OUT + PID_AD_TURN.OUT; //电磁分
    }
/*
    if (AD_Miss == 1) //出了赛道
    {
